...
public:
  ...
  {
    trials_csv_ = node_->get_parameter("grasp_seeded_trials_csv").as_string();
    trajectory_csv_ = node_->get_parameter("grasp_seeded_trajectory_csv").as_string();
    waypoints_yaml_ = node_->get_parameter("grasp_seeded_waypoints_yaml").as_string();
    audit_md_ = node_->get_parameter("grasp_seeded_audit_md").as_string();
  ...
  bool run()
  {
    // The first five trials test the best GRASP branch at each requested Lift;
    // the next five test the second-ranked branch without exceeding ten full tasks.
    const std::vector<double> lifts{ 0.25, 0.35, 0.30, 0.20, 0.40,
                                     0.25, 0.35, 0.30, 0.20, 0.40 };
    std::vector<TrialRecord> records;
    for (std::size_t index = 0; index < lifts.size(); ++index)
    {
      const int branch_rank = index < 5 ? 0 : 1;
      TrialRecord record = runTrial(static_cast<int>(index + 1), lifts[index], branch_rank);
      appendTrial(record);
      records.push_back(record);
      RCLCPP_INFO(node_->get_logger(),
                  "GRASP_SEEDED_PROGRESS trial=%d lift=%.2f branch=%d success=%s stage=%s waypoint=%d fraction=%.6f spacing=%.4f",
      ...
      {
        selected_ = std::make_unique<Trial>(record.trial);
        writeTrajectory(*selected_);
        writeWaypoints(*selected_);
        writeAudit(records, &record);
        publish(*selected_);
        return true;
      }
    }
...
private:
                       ...
                       std::string& collision_pairs)
  {
    ...
    segment.method = "PREVIOUS_STATE_SEEDED_CONTINUOUS_IK_MAX_" + std::to_string(spacing) + "M";
  ...
  TrialRecord runTrial(int trial_id, double lift, int branch_rank)
  {
    ...
    Candidate candidate;
    candidate.id = "grasp_seeded_" + std::to_string(trial_id);
    candidate.lift = lift;
    ...
    descent.method = "REVERSED_GRASP_SEEDED_CONTINUOUS_IK_MAX_" +
  ...
  void writeWaypoints(const Trial& trial) const
  {
    ...
    out << "status: GRASP_SEEDED_REVERSE_CONTINUATION_REFERENCE\n"
  ...
  void writeAudit(const std::vector<TrialRecord>& records, const TrialRecord* selected) const
  {
    ...
    out << "# Grasp-seeded reverse-continuation reference audit\n\n"
    ...
    {
      out << "`NO_APPROVABLE_GRASP_SEEDED_REFERENCE`\n\n"
...
int main(int argc, char** argv)
{
  ...
  auto node = std::make_shared<rclcpp::Node>("grasp_seeded_reference_generator", options);
  ...
  try
  {
    grasp_seeded::Runner runner(node);
