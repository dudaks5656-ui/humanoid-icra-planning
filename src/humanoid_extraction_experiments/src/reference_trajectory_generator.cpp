...
struct SceneConfig
{
  ...
  double grasp_height{};
  double q_contact{};
  double finger_bottom_tcp_offset{};
  double rim_clearance{};
  double transfer_outside_clearance{};
};
...
SceneConfig loadSceneConfig(const std::string& path)
{
  ...
  SceneConfig config;
  config.top_open_reference = root["top_open_reference"] && root["top_open_reference"].as<bool>();
  ...
  if (config.top_open_reference)
  {
    config.grasp_height = root["task"]["grasp_height_above_object_bottom"].as<double>();
    config.q_contact = root["task"]["q_contact_50mm"].as<double>();
    config.finger_bottom_tcp_offset = root["task"]["finger_bottom_tcp_offset"].as<double>();
    config.rim_clearance = root["task"]["rim_clearance"].as<double>();
    config.transfer_outside_clearance = root["task"]["transfer_outside_clearance"].as<double>();
  }
  return config;
}
...
struct CandidateResult
{
  ...
  std::vector<std::string> trajectory_stages;
...
struct DenseAudit
{
  bool valid{ false };
  std::string failure;
  std::string violating_joint;
  double violating_value{ std::numeric_limits<double>::quiet_NaN() };
  ...
  std::size_t violation_state_index{ 0 };
  std::size_t original_points{ 0 };
  std::size_t interpolated_states{ 0 };
  double min_all_joint_margin{ std::numeric_limits<double>::infinity() };
  double min_active_joint_margin{ std::numeric_limits<double>::infinity() };
  double min_environment_clearance{ std::numeric_limits<double>::infinity() };
  double min_self_clearance{ std::numeric_limits<double>::infinity() };
  double path_length{ 0.0 };
  double planning_time_ms{ 0.0 };
...
struct ReferenceCandidate
{
  ...
  explicit ReferenceCandidate(const moveit::core::RobotModelConstPtr& model) : result(model)
  {
  }
};
...
{
public:
  explicit ReferenceTrajectoryGenerator(const rclcpp::Node::SharedPtr& node)
    : node_(node), scene_config_(loadSceneConfig(parameter<std::string>("scene_config"))),
      ...
      reference_trials_csv_(parameter<std::string>("reference_trials_csv")),
      reference_trajectory_csv_(parameter<std::string>("reference_trajectory_csv")),
      reference_waypoints_yaml_(parameter<std::string>("reference_waypoints_yaml")),
      reference_audit_md_(parameter<std::string>("reference_audit_md")),
      hold_for_rviz_(parameter<bool>("hold_for_rviz"))
  {
    ...
    left_arm_with_torso_group_ = requiredGroup("left_arm_with_torso");
    ...
    marker_publisher_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>(
        "/reference_markers", rclcpp::QoS(1).transient_local().reliable());
    ...
    planning_group_name_ = scene_config_.top_open_reference ? "left_arm_with_torso" : "left_arm";
    move_group_ = std::make_unique<moveit::planning_interface::MoveGroupInterface>(
        node_, moveit::planning_interface::MoveGroupInterface::Options(planning_group_name_, "robot_description", ""),
    ...
    if (scene_config_.top_open_reference)
    {
      moveit_msgs::msg::Constraints torso_lock;
      torso_lock.name = "LIFT_ONLY_YAW_PITCH_LOCKED_ZERO";
      for (const std::string joint_name : { "waist_yaw_joint", "waist_pitch_joint" })
      {
        moveit_msgs::msg::JointConstraint constraint;
        constraint.joint_name = joint_name;
        constraint.position = 0.0;
        constraint.tolerance_above = 1e-9;
        constraint.tolerance_below = 1e-9;
        constraint.weight = 1.0;
        torso_lock.joint_constraints.push_back(constraint);
      }
      move_group_->setPathConstraints(torso_lock);
    }
    ...
    if (scene_config_.top_open_reference)
    {
      resolved_extraction_clearance_ = scene_config_.transfer_outside_clearance;
      resolved_pregrasp_clearance_ = scene_config_.rim_clearance;
    }
    else
    {
    ...
    RCLCPP_INFO(node_->get_logger(),
                "Offline reference generator ready: group=%s eef=%s pipeline=ompl "
                "planner=RRTConnectkConfigDefault top_open=%s pre_grasp_clearance=%.3f extraction_clearance=%.3f",
                planning_group_name_.c_str(), left_tcp_link_.c_str(),
                scene_config_.top_open_reference ? "true" : "false",
                resolved_pregrasp_clearance_, resolved_extraction_clearance_);
  }

  bool run()
  {
    return runReferenceGeneration();
  }
...
private:
  ...
  double variableMargin(const std::string& name, double value) const
  {
  ...
  void updateMarginsAndClearance(const moveit::core::RobotState& state,
                                 const std::vector<std::string>& active_names, DenseAudit& audit) const
  {
    for (const auto& name : whole_body_group_->getVariableNames())
      audit.min_all_joint_margin = std::min(audit.min_all_joint_margin,
                                            variableMargin(name, state.getVariablePosition(name)));
    for (const auto& name : active_names)
      audit.min_active_joint_margin = std::min(audit.min_active_joint_margin,
                                               variableMargin(name, state.getVariablePosition(name)));
    const auto clearance = stateClearances(state);
    audit.min_environment_clearance = std::min(audit.min_environment_clearance, clearance.first);
    audit.min_self_clearance = std::min(audit.min_self_clearance, clearance.second);
  }

  bool captureBoundsFailure(const moveit::core::RobotState& state, DenseAudit& audit,
                            std::size_t state_index) const
  {
    for (const auto& name : whole_body_group_->getVariableNames())
    {
      ...
      if (value < bounds.min_position_)
        excess = bounds.min_position_ - value;
      else if (value > bounds.max_position_)
        excess = value - bounds.max_position_;
      if (excess > 0.0)
      {
        audit.violating_joint = name;
        audit.violating_value = value;
        audit.lower_limit = bounds.min_position_;
        audit.upper_limit = bounds.max_position_;
        audit.excess = excess;
        audit.violation_state_index = state_index;
        return true;
      }
    }
    return false;
  ...
  std::size_t interpolationSteps(const moveit::core::RobotState& from,
                                 const trajectory_msgs::msg::JointTrajectoryPoint& to,
                                 const std::vector<std::string>& names) const
  {
    std::size_t steps = 1;
    for (std::size_t i = 0; i < names.size(); ++i)
    {
      const auto* joint = robot_model_->getJointOfVariable(names[i]);
      const double resolution = joint->getType() == moveit::core::JointModel::PRISMATIC ? 0.005 : 0.01;
      const double delta = std::abs(to.positions[i] - from.getVariablePosition(names[i]));
      steps = std::max(steps, static_cast<std::size_t>(std::ceil(delta / resolution)));
    }
    return steps;
  }

  DenseAudit denseValidate(const CandidateResult& result)
  {
    DenseAudit audit;
    audit.planning_time_ms = result.total_planning_time_ms;
    for (const auto& trajectory : result.trajectories)
    {
      audit.original_points += trajectory.joint_trajectory.points.size();
      audit.path_length += pathLength(trajectory);
    }
    if (!result.success || result.trajectories.size() != 5 || result.trajectory_stages.size() != 5)
    {
      audit.failure = result.first_failure_stage.empty() ? "INCOMPLETE_FIVE_STAGE_PATH" : result.first_failure_stage;
      return audit;
    }

    resetSceneForCandidate();
    moveit::core::RobotState current(robot_model_);
    if (!moveit::core::robotStateMsgToRobotState(result.start_state, current))
    {
      audit.failure = "START_STATE_DESERIALIZATION_FAILURE";
      return audit;
    }
    current.update();
    std::size_t dense_index = 0;

    for (std::size_t segment = 0; segment < result.trajectories.size(); ++segment)
    {
      const std::string& stage = result.trajectory_stages[segment];
      if (stage == "GRASP")
        setFingerTargetContactAllowed(true);
      const auto& trajectory = result.trajectories[segment].joint_trajectory;
      if (trajectory.joint_names.empty() || trajectory.points.empty())
      {
        audit.failure = "EMPTY_TRAJECTORY_SEGMENT:" + stage;
        return audit;
      }

      double start_discontinuity = 0.0;
      for (std::size_t j = 0; j < trajectory.joint_names.size(); ++j)
        start_discontinuity = std::max(start_discontinuity,
                                       std::abs(current.getVariablePosition(trajectory.joint_names[j]) -
                                                trajectory.points.front().positions[j]));
      if (start_discontinuity > 1e-6)
      {
        audit.failure = "STAGE_START_DISCONTINUITY:" + stage;
        return audit;
      }
      ...
      for (const auto& point : trajectory.points)
      {
        ...
        {
          audit.failure = "TRAJECTORY_DIMENSION_MISMATCH:" + stage;
          return audit;
        }
        const moveit::core::RobotState previous = current;
        const std::size_t steps = interpolationSteps(previous, point, trajectory.joint_names);
        for (std::size_t interpolation = 1; interpolation <= steps; ++interpolation)
        {
          current = previous;
          const double ratio = static_cast<double>(interpolation) / static_cast<double>(steps);
          std::vector<double> values;
          values.reserve(trajectory.joint_names.size());
          for (std::size_t j = 0; j < trajectory.joint_names.size(); ++j)
          {
            const double from = previous.getVariablePosition(trajectory.joint_names[j]);
            values.push_back(from + ratio * (point.positions[j] - from));
          }
          current.setVariablePositions(trajectory.joint_names, values);
          current.update();
          ++dense_index;
          ++audit.interpolated_states;
          updateMarginsAndClearance(current, trajectory.joint_names, audit);
          const CollisionStatus status = checkState(current);
          audit.attached_object_box_collision = audit.attached_object_box_collision ||
                                                hasAttachedObjectBoxCollision(status);
          if (!status.joint_limit_valid)
          {
            captureBoundsFailure(current, audit, dense_index);
            audit.failure = "JOINT_LIMIT_VIOLATION:" + audit.violating_joint;
            return audit;
          }
          if (status.self_collision)
          {
            audit.failure = "SELF_COLLISION:" + pairString(status.self_pairs);
            return audit;
          }
          if (status.environment_collision)
          {
            audit.failure = collisionFailure(status);
            return audit;
          }
        }
      }
  ...
  DenseAudit inspectRawBounds(const CandidateResult& result) const
  {
    DenseAudit audit;
    moveit::core::RobotState state(robot_model_);
    if (!moveit::core::robotStateMsgToRobotState(result.start_state, state))
    {
      audit.failure = "START_STATE_DESERIALIZATION_FAILURE";
      return audit;
    }
    ...
    for (const auto& trajectory : result.trajectories)
    {
      ...
      for (const auto& point : trajectory.joint_trajectory.points)
      {
        ...
        state.update();
        if (!state.satisfiesBounds(whole_body_group_))
        {
          captureBoundsFailure(state, audit, index);
          audit.failure = "JOINT_LIMIT_VIOLATION:" + audit.violating_joint;
          return audit;
        }
      }
  ...
  std::string excessClassification(double excess) const
  {
    if (!std::isfinite(excess))
      return "NOT_APPLICABLE";
    if (std::abs(excess) <= 1e-6)
      return "NUMERICAL_TOLERANCE_ONLY";
    if (std::abs(excess) <= 1e-4)
      return "SMALL_BUT_REAL_VIOLATION";
    return "INVALID_TRAJECTORY";
  }

  void initializeReferenceTrials() const
  {
    std::ofstream out(reference_trials_csv_, std::ios::trunc);
    if (!out)
      throw std::runtime_error("Cannot create reference trials CSV: " + reference_trials_csv_);
    out << "timestamp,trial_id,lift,yaw,pitch,planning_time_limit_s,planning_attempts,full_task_planning_success,"
           "first_failure_stage,dense_validation_performed,dense_valid,dense_failure,original_points,"
           "interpolated_states,min_all_joint_margin,min_active_joint_margin,min_environment_clearance,"
           "min_self_clearance,joint_path_length,total_planning_time_ms,attached_object_box_collision,"
           "violating_joint,lower_limit,upper_limit,trajectory_value,excess,violation_state_index,"
           "violation_classification\n";
  }

  void appendReferenceTrial(int trial_id, const CandidateResult& result, const DenseAudit& audit,
                            bool dense_performed) const
  {
    std::ofstream out(reference_trials_csv_, std::ios::app);
    out << csvEscape(timestampNow()) << ',' << trial_id << ',' << std::setprecision(15)
        << result.candidate.lift << ',' << result.candidate.yaw << ',' << result.candidate.pitch << ",30,10,"
        << (result.success ? 1 : 0) << ',' << csvEscape(result.first_failure_stage) << ','
        << (dense_performed ? 1 : 0) << ',' << (audit.valid ? 1 : 0) << ',' << csvEscape(audit.failure) << ','
        << audit.original_points << ',' << audit.interpolated_states << ',' << audit.min_all_joint_margin << ','
        << audit.min_active_joint_margin << ',' << audit.min_environment_clearance << ','
        << audit.min_self_clearance << ',' << audit.path_length << ',' << result.total_planning_time_ms << ','
        << (audit.attached_object_box_collision ? 1 : 0) << ',' << csvEscape(audit.violating_joint) << ','
        << audit.lower_limit << ',' << audit.upper_limit << ',' << audit.violating_value << ',' << audit.excess << ','
        << audit.violation_state_index << ',' << csvEscape(excessClassification(audit.excess)) << '\n';
  }

  bool betterReference(const ReferenceCandidate& a, const ReferenceCandidate& b) const
  {
    if (std::abs(a.audit.min_active_joint_margin - b.audit.min_active_joint_margin) > 1e-12)
      return a.audit.min_active_joint_margin > b.audit.min_active_joint_margin;
    if (scene_config_.top_open_reference)
    {
      if (std::abs(a.audit.min_self_clearance - b.audit.min_self_clearance) > 1e-12)
        return a.audit.min_self_clearance > b.audit.min_self_clearance;
      if (std::abs(a.audit.min_environment_clearance - b.audit.min_environment_clearance) > 1e-12)
        return a.audit.min_environment_clearance > b.audit.min_environment_clearance;
      if (std::abs(a.audit.path_length - b.audit.path_length) > 1e-12)
        return a.audit.path_length < b.audit.path_length;
      return a.audit.planning_time_ms < b.audit.planning_time_ms;
    }
    const double a_clearance = std::min(a.audit.min_environment_clearance, a.audit.min_self_clearance);
    const double b_clearance = std::min(b.audit.min_environment_clearance, b.audit.min_self_clearance);
    if (std::abs(a_clearance - b_clearance) > 1e-12)
      return a_clearance > b_clearance;
    if (std::abs(a.audit.path_length - b.audit.path_length) > 1e-12)
      return a.audit.path_length < b.audit.path_length;
    return a.audit.planning_time_ms < b.audit.planning_time_ms;
  }

  std::vector<std::pair<std::string, geometry_msgs::msg::Pose>> allStagePoses(
      const CandidateResult& result) const
  {
    auto poses = preExtractionStagePoses();
    if (scene_config_.top_open_reference)
      poses.push_back({ "TRANSFER_OUTSIDE", transferOutsidePose(result.attached_target_in_tcp) });
    else
      poses.push_back({ "EXTRACTION", extractionPose(resolved_extraction_clearance_,
                                                      result.attached_target_in_tcp) });
    return poses;
  }
  ...
  void writeReferenceTrajectory(const CandidateResult& result)
  {
    std::ofstream out(reference_trajectory_csv_, std::ios::trunc);
    if (!out)
      throw std::runtime_error("Cannot create reference trajectory CSV: " + reference_trajectory_csv_);
    out << "stage,trajectory_segment,point_index,time_from_start";
    for (const auto& name : whole_body_group_->getVariableNames())
      out << ',' << name;
    out << ",eef_x,eef_y,eef_z,eef_qx,eef_qy,eef_qz,eef_qw,lift,yaw,pitch,target_attached,"
           "bounds_valid,self_collision,environment_collision,collision_valid\n";

    resetSceneForCandidate();
    moveit::core::RobotState state(robot_model_);
    if (!moveit::core::robotStateMsgToRobotState(result.start_state, state))
      throw std::runtime_error("Cannot deserialize selected reference start state");
    for (std::size_t segment = 0; segment < result.trajectories.size(); ++segment)
    {
      const std::string& stage = result.trajectory_stages[segment];
      if (stage == "GRASP")
      ...
      for (std::size_t point_index = 0; point_index < trajectory.points.size(); ++point_index)
      {
        ...
        const Eigen::Isometry3d& tcp = state.getGlobalLinkTransform(left_tcp_link_);
        Eigen::Quaterniond q(tcp.rotation());
        q.normalize();
        out << stage << ',' << segment + 1 << ',' << point_index << ',' << std::setprecision(15)
            << pointTime(point);
        for (const auto& name : whole_body_group_->getVariableNames())
        ...
        out << ',' << tcp.translation().x() << ',' << tcp.translation().y() << ',' << tcp.translation().z()
            ...
            << ((stage == "LIFT" || stage == "EXTRACTION" || stage == "LIFT_CLEAR" ||
                 stage == "TRANSFER_OUTSIDE") ? 1 : 0) << ','
            << (status.joint_limit_valid ? 1 : 0) << ',' << (status.self_collision ? 1 : 0) << ','
            << (status.environment_collision ? 1 : 0) << ','
            << (status.joint_limit_valid && !status.self_collision && !status.environment_collision ? 1 : 0)
            << '\n';
      }
      if (stage == "GRASP")
        attachTargetAtomically(state);
    }
  }

  void writePoseYaml(std::ostream& out, const geometry_msgs::msg::Pose& pose, const std::string& indent) const
  {
    out << indent << "xyz: [" << std::setprecision(15) << pose.position.x << ", " << pose.position.y << ", "
        << pose.position.z << "]\n";
    out << indent << "quaternion_xyzw: [" << pose.orientation.x << ", " << pose.orientation.y << ", "
        << pose.orientation.z << ", " << pose.orientation.w << "]\n";
  }

  void writeStateYaml(std::ostream& out, const moveit::core::RobotState& state,
                      const std::string& indent) const
  {
    for (const auto& name : whole_body_group_->getVariableNames())
      out << indent << name << ": " << std::setprecision(15) << state.getVariablePosition(name) << '\n';
  }

  void writeReferenceWaypoints(const CandidateResult& result)
  {
    std::ofstream out(reference_waypoints_yaml_, std::ios::trunc);
    if (!out)
      throw std::runtime_error("Cannot create reference waypoint YAML: " + reference_waypoints_yaml_);
    out << "status: " << (scene_config_.top_open_reference ?
      "TOP_OPEN_OFFLINE_REFERENCE_TRAJECTORY" : "OFFLINE_REFERENCE_TRAJECTORY") << "\n";
    out << "scene_status: " << (scene_config_.top_open_reference ?
      "TOP_OPEN_BOX_600X400X150" : "PROVISIONAL_METHOD_DEVELOPMENT_ONLY") << "\n";
    if (scene_config_.top_open_reference)
      out << "reference_grasp: KINEMATIC_REFERENCE_GRASP_50MM\n"
             "grasp_qualification: KINEMATIC_PATH_PLANNING_GRASP_ONLY\n"
             "force_closure_validated: false\n"
             "hardware_grasp_stability_validated: false\n";
    out << "target_pose:\n";
    geometry_msgs::msg::Pose target;
    target.position.x = scene_config_.target_position[0];
    target.position.y = scene_config_.target_position[1];
    target.position.z = scene_config_.target_position[2];
    target.orientation.w = 1.0;
    writePoseYaml(out, target, "  ");
    out << "approved_end_effector_orientation_rpy: [" << scene_config_.eef_rpy[0] << ", "
        << scene_config_.eef_rpy[1] << ", " << scene_config_.eef_rpy[2] << "]\n";
    out << "selected_torso:\n  lift: " << result.candidate.lift << "\n  yaw: " << result.candidate.yaw
        << "\n  pitch: " << result.candidate.pitch << "\n";
    out << "tcp_to_target_attached_transform:\n  xyz: ["
        << result.attached_target_in_tcp.translation().x() << ", "
        << result.attached_target_in_tcp.translation().y() << ", "
        << result.attached_target_in_tcp.translation().z() << "]\n";
    Eigen::Quaterniond attached_q(result.attached_target_in_tcp.rotation());
    attached_q.normalize();
    out << "  quaternion_xyzw: [" << attached_q.x() << ", " << attached_q.y() << ", "
        << attached_q.z() << ", " << attached_q.w() << "]\n";
    out << "stage_goal_poses:\n";
    for (const auto& stage_pose : allStagePoses(result))
    {
      out << "  " << stage_pose.first << ":\n";
      writePoseYaml(out, stage_pose.second, "    ");
    }

    moveit::core::RobotState state(robot_model_);
    if (!moveit::core::robotStateMsgToRobotState(result.start_state, state))
      throw std::runtime_error("Cannot deserialize selected reference start state for YAML");
    out << "stage_joint_states:\n";
    for (std::size_t segment = 0; segment < result.trajectories.size(); ++segment)
    {
      out << "  " << result.trajectory_stages[segment] << ":\n    start:\n";
      writeStateYaml(out, state, "      ");
      const auto& trajectory = result.trajectories[segment].joint_trajectory;
      const auto& final_point = trajectory.points.back();
      state.setVariablePositions(trajectory.joint_names, final_point.positions);
      state.update();
      out << "    end:\n";
      writeStateYaml(out, state, "      ");
    }
  ...
  void writeReferenceAudit(const std::vector<std::unique_ptr<ReferenceCandidate>>& valid,
                           int executed_trials, const CandidateResult* selected) const
  {
    std::ofstream out(reference_audit_md_, std::ios::trunc);
    if (!out)
      throw std::runtime_error("Cannot create reference audit: " + reference_audit_md_);
    out << "# " << (scene_config_.top_open_reference ? "Top-open offline reference path audit" :
                     "Offline reference path audit") << "\n\n";
    out << "- Scene: " << (scene_config_.top_open_reference ?
      "TOP_OPEN_BOX_600X400X150 with front/back/left/right walls, floor, and open top" :
      "provisional method-development confined scene") << "\n";
    out << "- Target XYZ: [" << scene_config_.target_position[0] << ", "
        << scene_config_.target_position[1] << ", " << scene_config_.target_position[2] << "] m\n";
    out << "- Mode: LIFT_ONLY; Yaw = 0; Pitch = 0\n";
    out << "- Planning limit: 30 s per stage; maximum 10 attempts\n";
    out << "- Full-task trials executed: " << executed_trials << "/20\n";
    out << "- Dense-valid candidates: " << valid.size() << "\n";
    out << "- Dense interpolation: revolute <= 0.01 rad, prismatic <= 0.005 m\n";
    out << "- Collision detector/distance source: MoveIt FCL PlanningScene (`distanceRobot`, `distanceSelf`)\n";
    out << "- q_open = 0.044 m is a simulation planning aperture, not a hardware-validated fully-open value.\n\n";
    if (scene_config_.top_open_reference)
      out << "- Fixed grasp: `KINEMATIC_REFERENCE_GRASP_50MM`, height=0.050 m, q_contact="
          << scene_config_.q_contact << " m\n"
             "- Qualification: KINEMATIC_PATH_PLANNING_GRASP_ONLY; NOT_FORCE_CLOSURE_VALIDATED; "
             "NOT_HARDWARE_GRASP_STABILITY_VALIDATED\n"
             "- Task stages: TOP_APPROACH, VERTICAL_DESCENT, GRASP, LIFT_CLEAR, TRANSFER_OUTSIDE\n"
             "- Minimum object-over-rim vertical clearance: " << scene_config_.rim_clearance << " m\n\n";
    out << "All-variable minimum margin is expected to be zero because q_open is exactly the independent finger "
           "upper bound. Selection therefore uses the minimum margin of the actively planned seven left-arm "
           "variables, while still requiring exact bounds validity for all 19 independent variables.\n\n";
    out << "| Trial | Lift | Active margin | Environment clearance | Self clearance | Path length | Planning ms |\n";
    out << "|---:|---:|---:|---:|---:|---:|---:|\n";
    for (const auto& candidate : valid)
      out << '|' << candidate->trial_id << '|' << candidate->result.candidate.lift << '|'
          << candidate->audit.min_active_joint_margin << '|' << candidate->audit.min_environment_clearance << '|'
          << candidate->audit.min_self_clearance << '|' << candidate->audit.path_length << '|'
          << candidate->audit.planning_time_ms << "|\n";
    if (!selected)
    {
      out << "\nResult: FAILED_TO_OBTAIN_FIVE_DENSE_VALID_CANDIDATES. No reference was selected.\n";
      return;
    }
    out << "\nSelected trial: " << selected_reference_trial_ << "\n";
    out << "Selected Lift/Yaw/Pitch: " << selected->candidate.lift << " / " << selected->candidate.yaw << " / "
        << selected->candidate.pitch << "\n";
    out << "Selected minimum active-joint margin: " << selected_audit_.min_active_joint_margin << "\n";
    out << "Selected minimum environment clearance: " << selected_audit_.min_environment_clearance << "\n";
    out << "Selected minimum self clearance: " << selected_audit_.min_self_clearance << "\n";
    out << "Selected joint-space path length: " << selected_audit_.path_length << "\n";
    out << "Selected dense states checked: " << selected_audit_.interpolated_states << "\n";
    out << "Attached-object/box collision: "
        << (selected_audit_.attached_object_box_collision ? "DETECTED" : "NOT_DETECTED") << "\n\n";
    out << "All five stages are connected by the previous segment's final RobotState. The target remains a world "
           "object through GRASP, finger-target contact is allowed only at GRASP, and the target is attached for "
        << (scene_config_.top_open_reference ? "LIFT_CLEAR and TRANSFER_OUTSIDE" : "LIFT and EXTRACTION")
    ...
    if (scene_config_.top_open_reference)
    {
      ...
      {
        if (std::isfinite(stage.position_error))
          max_position_error = std::max(max_position_error, stage.position_error);
        if (std::isfinite(stage.orientation_error))
          max_orientation_error = std::max(max_orientation_error, stage.orientation_error);
      }
      out << "Maximum endpoint position error: " << max_position_error << " m\n";
      out << "Maximum endpoint orientation error: " << max_orientation_error << " rad (limit "
          << scene_config_.orientation_tolerance << " rad)\n";
    }
  }

  visualization_msgs::msg::Marker axisMarker(int id, const geometry_msgs::msg::Pose& pose,
                                              const Eigen::Vector3d& local_axis,
                                              float r, float g, float b) const
  {
    visualization_msgs::msg::Marker marker;
    marker.header.frame_id = scene_config_.frame_id;
    marker.header.stamp = node_->now();
    marker.ns = "reference_stage_axes";
    marker.id = id;
    ...
    const Eigen::Vector3d end_vector = q.toRotationMatrix() * local_axis * 0.06;
  ...
  void publishReferenceMarkers(const CandidateResult& result)
  {
    ...
    for (const auto& stage_pose : allStagePoses(result))
    {
      array.markers.push_back(axisMarker(id++, stage_pose.second, Eigen::Vector3d::UnitX(), 1.0f, 0.0f, 0.0f));
      array.markers.push_back(axisMarker(id++, stage_pose.second, Eigen::Vector3d::UnitY(), 0.0f, 1.0f, 0.0f));
      array.markers.push_back(axisMarker(id++, stage_pose.second, Eigen::Vector3d::UnitZ(), 0.0f, 0.3f, 1.0f));
      visualization_msgs::msg::Marker label;
      label.header.frame_id = scene_config_.frame_id;
      label.header.stamp = node_->now();
      label.ns = "reference_stage_labels";
      label.id = id++;
      label.type = visualization_msgs::msg::Marker::TEXT_VIEW_FACING;
      label.action = visualization_msgs::msg::Marker::ADD;
      label.pose = stage_pose.second;
      ...
      label.color.b = 1.0;
      label.color.a = 1.0;
      label.text = stage_pose.first;
      array.markers.push_back(label);
    }

    visualization_msgs::msg::Marker title;
    title.header.frame_id = scene_config_.frame_id;
    title.header.stamp = node_->now();
    title.ns = "reference_title";
    title.id = id++;
    title.type = visualization_msgs::msg::Marker::TEXT_VIEW_FACING;
    title.action = visualization_msgs::msg::Marker::ADD;
    title.pose.position.x = scene_config_.box_center[0];
    title.pose.position.y = scene_config_.box_center[1];
    title.pose.position.z = scene_config_.box_center[2] + scene_config_.box_height / 2.0 + 0.15;
    ...
    title_text << (scene_config_.top_open_reference ?
      "KINEMATIC REFERENCE GRASP — FORCE NOT VALIDATED | TOP-OPEN OFFLINE REFERENCE TRAJECTORY  Lift=" :
      "OFFLINE REFERENCE TRAJECTORY  Lift=") << result.candidate.lift
               << " Yaw=" << result.candidate.yaw << " Pitch=" << result.candidate.pitch;
    title.text = title_text.str();
    array.markers.push_back(title);

    visualization_msgs::msg::Marker object_path;
    object_path.header.frame_id = scene_config_.frame_id;
    object_path.header.stamp = node_->now();
    object_path.ns = "attached_object_path";
    object_path.id = id++;
    object_path.type = visualization_msgs::msg::Marker::LINE_STRIP;
    object_path.action = visualization_msgs::msg::Marker::ADD;
    object_path.scale.x = 0.01;
    object_path.color.r = 0.0;
    object_path.color.g = 1.0;
    object_path.color.b = 0.0;
    object_path.color.a = 1.0;
    moveit::core::RobotState state(robot_model_);
    moveit::core::robotStateMsgToRobotState(result.start_state, state);
    bool attached = false;
    for (std::size_t segment = 0; segment < result.trajectories.size(); ++segment)
    {
      const auto& trajectory = result.trajectories[segment].joint_trajectory;
      for (const auto& point : trajectory.points)
      {
        ...
        if (attached)
        {
          const Eigen::Isometry3d target_world = state.getGlobalLinkTransform(left_tcp_link_) *
                                                  result.attached_target_in_tcp;
          geometry_msgs::msg::Point p;
          p.x = target_world.translation().x();
          p.y = target_world.translation().y();
          p.z = target_world.translation().z();
          object_path.points.push_back(p);
        }
      }
      if (result.trajectory_stages[segment] == "GRASP")
        attached = true;
    }
    array.markers.push_back(object_path);
    marker_publisher_->publish(array);
  }

  bool runReferenceGeneration()
  {
    scene_config_.planning_time = 30.0;
    scene_config_.planning_attempts = 10;
    move_group_->setPlanningTime(30.0);
    move_group_->setNumPlanningAttempts(10);
    if (!scene_config_.top_open_reference)
      configureTarget(0.50, 0.30);
    initializeReferenceTrials();

    const auto lifts = liftCandidates();
    if (lifts.empty())
      throw std::runtime_error("No valid Lift candidate is available");
    std::vector<std::unique_ptr<ReferenceCandidate>> valid;
    int executed_trials = 0;
    for (int trial_id = 1; trial_id <= 20 && valid.size() < 5; ++trial_id)
    {
      ++executed_trials;
      Candidate candidate;
      candidate.id = "offline_reference_trial_" + std::to_string(trial_id);
      candidate.lift = lifts[static_cast<std::size_t>(trial_id - 1) % lifts.size()];
      candidate.yaw = 0.0;
      candidate.pitch = 0.0;
      candidate.yaw_deg = 0.0;
      candidate.pitch_deg = 0.0;
      CandidateResult result = runCandidate("LIFT_ONLY", candidate);
      DenseAudit audit = result.success ? denseValidate(result) : inspectRawBounds(result);
      appendReferenceTrial(trial_id, result, audit, result.success);
      if (result.success && audit.valid)
      {
        auto accepted = std::make_unique<ReferenceCandidate>(robot_model_);
        accepted->trial_id = trial_id;
        accepted->result = std::move(result);
        accepted->audit = audit;
        valid.push_back(std::move(accepted));
      }
      RCLCPP_INFO(node_->get_logger(), "REFERENCE_PROGRESS trial=%d valid=%zu/5", trial_id, valid.size());
    }

    if (valid.size() < 5)
    {
      writeReferenceAudit(valid, executed_trials, nullptr);
      return false;
    }
    std::stable_sort(valid.begin(), valid.end(),
                     [this](const auto& a, const auto& b) { return betterReference(*a, *b); });
    selected_reference_trial_ = valid.front()->trial_id;
    selected_audit_ = valid.front()->audit;
    selected_result_ = std::make_unique<CandidateResult>(std::move(valid.front()->result));
    writeReferenceTrajectory(*selected_result_);
    writeReferenceWaypoints(*selected_result_);
    writeReferenceAudit(valid, executed_trials, selected_result_.get());
    publishResult(*selected_result_);
    publishReferenceMarkers(*selected_result_);
    return true;
  }
  ...
  std::vector<moveit_msgs::msg::CollisionObject> makeSceneObjects() const
  {
    ...
    objects.push_back(boxObject("box_bottom", { d + (scene_config_.top_open_reference ? 2.0 * t : t),
                                                 w + 2.0 * t, f },
                                { c[0], c[1], c[2] - h / 2.0 - f / 2.0 }));
    ...
    if (scene_config_.top_open_reference)
      objects.push_back(boxObject("box_front_wall", { t, w + 2.0 * t, h },
                                  { c[0] - d / 2.0 - t / 2.0, c[1], c[2] }));
  ...
  geometry_msgs::msg::Pose graspPose() const
  {
    if (scene_config_.top_open_reference)
    {
      const double object_bottom = scene_config_.target_position[2] - scene_config_.target_size[2] / 2.0;
      return makePose(scene_config_.target_position[0], scene_config_.target_position[1],
                      object_bottom + scene_config_.grasp_height + scene_config_.tcp_to_grasp_center);
    }
  ...
  geometry_msgs::msg::Pose approachPose(double clearance) const
  {
    ...
    if (scene_config_.top_open_reference)
      pose.position.z = scene_config_.box_center[2] + scene_config_.box_height / 2.0 + clearance +
                        scene_config_.finger_bottom_tcp_offset;
    else
      pose.position.x -= clearance;
    return pose;
  }

  geometry_msgs::msg::Pose liftClearPose() const
  {
    geometry_msgs::msg::Pose pose = graspPose();
    const double box_top = scene_config_.box_center[2] + scene_config_.box_height / 2.0;
    const double desired_object_center_z = box_top + scene_config_.rim_clearance +
                                           scene_config_.target_size[2] / 2.0;
    pose.position.z += desired_object_center_z - scene_config_.target_position[2];
    return pose;
  }

  geometry_msgs::msg::Pose transferOutsidePose(const Eigen::Isometry3d& target_in_tcp) const
  {
    const double front_inner = scene_config_.box_center[0] - scene_config_.box_depth / 2.0;
    const double front_outer = front_inner - scene_config_.wall_thickness;
    const double object_center_x = front_outer - scene_config_.transfer_outside_clearance -
                                   scene_config_.target_size[0] / 2.0;
    const double box_top = scene_config_.box_center[2] + scene_config_.box_height / 2.0;
    Eigen::Isometry3d target_goal = Eigen::Isometry3d::Identity();
    target_goal.translation() = Eigen::Vector3d(
      object_center_x, scene_config_.target_position[1],
      box_top + scene_config_.rim_clearance + scene_config_.target_size[2] / 2.0);
  ...
  std::vector<std::pair<std::string, geometry_msgs::msg::Pose>> preExtractionStagePoses() const
  {
    ...
    {
      return {
        { "TOP_APPROACH", approachPose(scene_config_.rim_clearance) },
        { "VERTICAL_DESCENT", grasp },
        { "GRASP", grasp },
        { "LIFT_CLEAR", liftClearPose() },
      };
    }
  ...
  bool findCollisionFreeIk(const std::string& mode, const Candidate& candidate, const std::string& stage,
                           ...
                           StageResult& result, moveit::core::RobotState* selected_state) const
  {
    bool any_ik = false;
    ...
    std::vector<double> lift_values{ candidate.lift };
    if (scene_config_.top_open_reference)
    {
      lift_values.clear();
      const auto& bounds = robot_model_->getVariableBounds("lift_joint");
      for (double lift = bounds.min_position_; lift <= bounds.max_position_ + 1e-12;
           lift += scene_config_.lift_step)
        lift_values.push_back(std::min(lift, bounds.max_position_));
      const double current_lift = current.getVariablePosition("lift_joint");
      std::stable_sort(lift_values.begin(), lift_values.end(), [current_lift](double a, double b) {
        return std::abs(a - current_lift) < std::abs(b - current_lift);
      });
    }

    int global_seed_id = 0;
    for (const double lift_value : lift_values)
    {
      Candidate effective_candidate = candidate;
      effective_candidate.lift = lift_value;
      for (int seed_id = 0; seed_id < scene_config_.ik_multistart_count; ++seed_id, ++global_seed_id)
      {
        moveit::core::RobotState ik_state = current;
        ik_state.setVariablePosition("lift_joint", lift_value);
        if (scene_config_.top_open_reference)
        {
          ik_state.setVariablePosition("waist_yaw_joint", 0.0);
          ik_state.setVariablePosition("waist_pitch_joint", 0.0);
        }
        if (seed_id > 0)
        {
          std::mt19937_64 rng(scene_config_.ik_rng_seed + stage_hash +
                              static_cast<std::uint64_t>(global_seed_id));
          for (const auto& variable : left_arm_group_->getVariableNames())
          {
            const auto& bounds = robot_model_->getVariableBounds(variable);
        ...
        if (ik)
        {
          ...
          if (status.joint_limit_valid && !status.self_collision && !status.environment_collision)
          {
            ...
            if (selected_state)
              *selected_state = ik_state;
          }
        }
        ++result.ik_seeds_tested;
        appendIkAudit(mode, effective_candidate, stage, global_seed_id, ik, status);
        if (any_valid)
          break;
      }
      if (any_valid)
                        ...
                        moveit_msgs::msg::RobotTrajectory& output_trajectory) const
  {
    ...
    moveit::core::RobotState ik_goal(robot_model_);
    if (!findCollisionFreeIk(mode, candidate, stage, pose, current, result, &ik_goal))
    {
    ...
    if (scene_config_.top_open_reference)
      move_group_->setJointValueTarget(ik_goal);
    else
    ...
    // Preserve the unmodified MoveIt trajectory before any post-plan rejection. This prevents
    // the raw-point loss that made the 22 pilot joint-limit failures non-auditable.
    ...
    if (scene_config_.top_open_reference &&
        (result.position_error > scene_config_.position_tolerance + 1e-12 ||
         result.orientation_error > scene_config_.orientation_tolerance + 1e-12))
    {
      result.planning_success = false;
      result.collision_free = false;
      result.failure_reason = result.orientation_error > scene_config_.orientation_tolerance + 1e-12 ?
  ...
  StageResult closeGripperStage(moveit::core::RobotState& current,
                                moveit_msgs::msg::RobotTrajectory& output_trajectory) const
  {
    StageResult result;
    result.stage = "GRASP";
    result.ik_seeds_tested = 0;
    result.ik_success = true;
    result.joint_limit_valid = true;
    output_trajectory.joint_trajectory.joint_names = { "openarm_left_finger_joint1" };
    trajectory_msgs::msg::JointTrajectoryPoint start;
    start.positions = { current.getVariablePosition("openarm_left_finger_joint1") };
    start.time_from_start.sec = 0;
    trajectory_msgs::msg::JointTrajectoryPoint contact;
    contact.positions = { scene_config_.q_contact };
    contact.time_from_start.sec = 1;
    output_trajectory.joint_trajectory.points = { start, contact };

    moveit::core::RobotState final_state(robot_model_);
    std::string failure;
    if (!trajectoryCollisionFree(current, output_trajectory, final_state, failure,
                                 result.minimum_environment_clearance, result.minimum_self_clearance,
                                 result.attached_object_box_collision))
    {
    ...
    result.trajectory_points = output_trajectory.joint_trajectory.points.size();
    result.joint_path_length = std::abs(start.positions[0] - contact.positions[0]);
    poseError(final_state, graspPose(), result.position_error, result.orientation_error);
    result.collision_free = true;
    result.planning_success = true;
    result.collision_free_ik_count = 1;
    current = final_state;
    return result;
  }
  ...
  CandidateResult runCandidate(const std::string& mode, const Candidate& candidate)
  {
    ...
    for (const auto& target : preExtractionStagePoses())
    {
      if (target.first == "GRASP")
        setFingerTargetContactAllowed(true);
      moveit_msgs::msg::RobotTrajectory trajectory;
      StageResult stage = (scene_config_.top_open_reference && target.first == "GRASP") ?
        closeGripperStage(current, trajectory) :
        planStage(mode, candidate, target.first, target.second, current, trajectory);
      ...
      if (!trajectory.joint_trajectory.points.empty())
      {
        ...
        result.trajectory_stages.push_back(target.first);
      }
      ...
      {
        attachTargetAtomically(current);
        result.attached_target_in_tcp = attached_target_in_tcp_;
      }
    }
    ...
    const std::string final_stage_name = scene_config_.top_open_reference ? "TRANSFER_OUTSIDE" : "EXTRACTION";
    const geometry_msgs::msg::Pose extraction = scene_config_.top_open_reference ?
      transferOutsidePose(attached_target_in_tcp_) :
      extractionPose(resolved_extraction_clearance_, attached_target_in_tcp_);
    StageResult extraction_stage = planStage(mode, candidate, final_stage_name, extraction, current,
                                             extraction_trajectory);
    ...
    if (!extraction_trajectory.joint_trajectory.points.empty())
    {
      result.trajectories.push_back(extraction_trajectory);
      result.trajectory_stages.push_back(final_stage_name);
    }
                    ...
                    std::size_t executed, std::size_t excluded, bool desired_case) const
  {
    ...
    out << "No trajectory was executed. No ros2_control, controller, hardware interface, or real robot node was used. "
           ...
           "through GRASP and was attached atomically for LIFT and EXTRACTION.\n";
  }
  ...
  std::string reference_trials_csv_;
  std::string reference_trajectory_csv_;
  std::string reference_waypoints_yaml_;
  std::string reference_audit_md_;
  ...
  std::string planning_group_name_{ "left_arm" };
  ...
  const moveit::core::JointModelGroup* left_arm_with_torso_group_{ nullptr };
  ...
  DenseAudit selected_audit_;
  int selected_reference_trial_{ 0 };
...
int main(int argc, char** argv)
{
  ...
  auto node = std::make_shared<rclcpp::Node>("reference_trajectory_generator", options);
  ...
  try
  {
    auto experiment = std::make_shared<ReferenceTrajectoryGenerator>(node);
  ...
  {
    std::cerr << "reference_trajectory_generator fatal: " << error.what() << std::endl;
  }
  rclcpp::shutdown();
  executor.cancel();
