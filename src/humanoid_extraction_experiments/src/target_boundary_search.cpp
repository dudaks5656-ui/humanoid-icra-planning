...
class TargetBoundarySearch
{
public:
  explicit TargetBoundarySearch(const rclcpp::Node::SharedPtr& node)
      ...
      hold_for_rviz_(parameter<bool>("hold_for_rviz"))
  {
    ...
    initializeCsv();
    initializeBoundaryCsvs();
    RCLCPP_INFO(node_->get_logger(),
                "Boundary search ready: group=left_arm eef=%s pipeline=ompl "
...
int main(int argc, char** argv)
{
  ...
  auto node = std::make_shared<rclcpp::Node>("target_boundary_search", options);
  ...
  try
  {
    auto experiment = std::make_shared<TargetBoundarySearch>(node);
